...
public class OradorDAOMysqlImpl implements iOradorDAO {
	...
	public Orador getById(Long id)  throws Exception{
		...
		if (resultset.next()){
			...
			String fechaBd = resultset.getString("fec_presentacion");
//			String activoBd = resultset.getString("activo");
			cerrar(connection);
			return new Orador(idBd,nombreBd,apellidoBd,mailBd,temaBd,fechaBd);
...
//			String activoBd = resultset.getString("activo");
			...
			Orador d = new Orador(idBd,nombreBd,apellidoBd,mailBd,temaBd,fechaBd);
			oradores.add(d);
	...
	public void update(Orador orador) throws Exception {
		...
		Connection connection = AdministradorDeConexiones.getConnection();
		//2 - arma el statement
		 String sql = "update oradores set nombre = ?, apellido = ?, mail = ?, tema = ?, fecha =? where id_orador= ?"  ;
		 PreparedStatement statement  = connection.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS);
	    statement.setString(1,orador.getNombre());
		 	statement.setString(2,orador.getApellido());
		 	statement.setString(3,orador.getMail());
		 	statement.setString(4,orador.getTema());
			statement.setString(5,orador.getFecha());
